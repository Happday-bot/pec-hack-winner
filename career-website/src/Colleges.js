  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const table =
          qualification === "10" ? "10th_profile_data" : "12th_profile_data";
        ...
        if (profileError || !profile) {
          ...
          if (
            ...
            profile.preferred_locations &&
            typeof profile.preferred_locations === "object"
          ) {
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 text-center max-w-md shadow-2xl">
            ...
            <p className="text-gray-600 mb-6">
              To view colleges based on your preferences, please complete your
              profile setup first.
            </p>
            <div className="flex gap-4 justify-center">
              ...
              <button
                ...
                className="bg-gray-100 text-gray-700 px-6 py-2 rounded-xl font-bold hover:bg-gray-200 transition"
              >
                Back
              </button>
            </div>
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-extrabold mb-8 flex items-center justify-center gap-3">
            Suggested Colleges{" "}
            <GraduationCap className="w-10 h-10 md:w-12 md:h-12 text-yellow-300" />
          </h1>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredColleges.map((college) => {
              ...
              return (
                <div
                  key={college.id}
                  className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col"
                >
                  <div className="p-6 flex-grow">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span className="flex items-center gap-1 text-xs font-semibold text-gray-500">
                        <MapPin className="w-3 h-3" /> {college.district},{" "}
                        {college.state}
                      </span>
                    </div>
      ...
      {selectedCollege && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative z-10 shadow-2xl animate-fade-in-up">
            {/* College image / banner at top with proper fit */}
            {selectedCollege.college_pic && (
              <div className="w-full h-56 bg-gray-100 rounded-t-3xl flex items-center justify-center">
                <img
                  src={selectedCollege.college_pic}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-indigo-50 p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="text-center">
                  <span className="block text-gray-500 font-bold uppercase text-xs mb-1">
                    Rank
                  </span>
                  <span className="font-semibold text-xl text-gray-800">
                    {selectedCollege.rank || "N/A"}
                  </span>
                </div>
                <div className="text-center">
                  <span className="block text-gray-500 font-bold uppercase text-xs mb-1">
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <p className="text-gray-600 flex items-center gap-1">
                      <MapPin className="w-4 h-4" /> {selectedCollege.district},{" "}
                      {selectedCollege.state}
                    </p>
                    {selectedCollege.geo && (
                      <p className="text-gray-500 text-xs">
                        Lat: {selectedCollege.geo.lat}, Long:{" "}
                        {selectedCollege.geo.long}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    {selectedCollege.contact &&
                      Array.isArray(selectedCollege.contact) &&
                      selectedCollege.contact.map((phone, i) => (
                        <p
                          key={i}
                    ...
                    {selectedCollege.email &&
                      Array.isArray(selectedCollege.email) &&
                      selectedCollege.email.map((email, i) => (
                        <p
                          key={i}
                          className="flex items-center gap-2 text-gray-700"
                        >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedCollege.degrees?.map((deg, i) => (
                      <span
                        key={i}
                        className="px-3 py-2 bg-indigo-100 text-indigo-700 rounded-lg text-sm font-medium border border-indigo-200"
                      >
                        {deg}
                      </span>
                    ))}
                  ...
                  {selectedCollege.duration && (
                    <p className="mt-3 text-sm text-gray-600 flex items-center gap-2">
                      <CircleAlert className="w-4 h-4" /> Duration:{" "}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t">
                  {selectedCollege.placements && (
                    <div>
                      ...
                      <p className="text-sm text-gray-700 bg-green-50 p-4 rounded-xl">
                        {selectedCollege.placements}
                      </p>
                    </div>
                  )}
                  ...
                  {selectedCollege.cutoff &&
                    Object.keys(selectedCollege.cutoff).length > 0 && (
                      <div>
                        <h3 className="text-lg font-bold text-gray-800 mb-3">
                          Cutoffs
                        </h3>
                        <div className="bg-gray-50 p-4 rounded-xl text-sm space-y-2">
                          {Object.entries(selectedCollege.cutoff).map(
                            ([exam, details]) => (
                              <div
                                key={exam}
                                className="p-3 bg-white rounded-lg border-l-4 border-indigo-500"
                              >
                                <span className="font-bold uppercase text-indigo-600 text-xs">
                                  {exam.replace("_", " ")}:
                                </span>
                                  ...
                                  {typeof details === "object" ? (
                                    Object.entries(details).map(
                                      ([cat, val]) => (
                                        <span
                                          key={cat}
                                          className="inline-block bg-indigo-100 text-indigo-800 px-2 py-1 rounded text-xs mr-2 mb-1"
                                        >
                                          {cat}: {val}
                                        </span>
                                      )
                  ...
                  {selectedCollege.career && (
                    <div className="lg:col-span-2">
                      ...
                      <p className="text-sm text-gray-700 bg-blue-50 p-4 rounded-xl">
                        {selectedCollege.career}
                      </p>
                    </div>
                  )}
                  ...
                  {selectedCollege.alumini && (
                    <div className="lg:col-span-2">
                      ...
                      <p className="text-sm text-gray-700 bg-purple-50 p-4 rounded-xl">
                        {selectedCollege.alumini}
                      </p>
                    </div>
                  )}
